...
if st.button("메모장 지우기"):
    st.session_state.memo_text = ""  # 메모장 내용만 초기화
...
if st.session_state.memo_text:
    ...
    for line in lines:
        ...
        if "이름:" in line:
            ...
            # 한국 자동차 번호판 패턴: [0-9]{2}[가-힣]{1}[0-9]{4}, [0-9]{2}[가-힣]{1}[0-9]{3}
            if re.match(r'^\d{2}[가-힣]{1}\d{4}$', car_number) or re.match(r'^\d{2}[가-힣]{1}\d{3}$', car_number):
