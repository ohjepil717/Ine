...
if st.session_state.memo_text:
    ...
    current_set = None  # 한 사람의 데이터를 저장
    ...
    for line in lines:
        ...
        if "이름:" in line:
            if current_set is not None:  # 이전 세트가 완성된 경우 저장
            ...
            if "날짜:" in line or "방문 날짜:" in line:
    ...
    # 마지막 세트 추가 (현재 세트가 존재하는 경우)
    if current_set is not None:
        categorized_data.append(current_set)

    # 세션 상태 초기화 후 새 데이터로 갱신
    st.session_state.data_list = categorized_data
    if categorized_data:
